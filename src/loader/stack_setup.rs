...
use crate::parse_elf::{ElfType, LoadInfo};
...
pub fn setup_stack(load_info: &LoadInfo, load_address: usize, interp_base: usize) -> usize {
    ...
    for arg in args.iter_mut().rev() {
        ...
        let argv_bytes = unsafe { arg.as_mut_vec() };
    ...
    unsafe {
        ...
        write_aux_val(
            &mut stack_pointer,
            AT_SYSINFO_EHDR,
            libc::getauxval(AT_SYSINFO_EHDR),
        );
        ...
        write_aux_val(
            &mut stack_pointer,
            AT_PHDR,
            (load_address + load_info.pheader_off) as u64,
        );
        ...
        match load_info.etype {
            ElfType::ElfExec => {
                write_aux_val(&mut stack_pointer, AT_ENTRY, load_info.entry_point as u64)
            }
            ElfType::ElfDyn => write_aux_val(
                &mut stack_pointer,
                AT_ENTRY,
                (load_address + load_info.entry_point) as u64,
            ),
        }
